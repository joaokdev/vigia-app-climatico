...
(() => {
  ...
  const originalCopy = {
    ...
    subtitleHTML:
      'Digite o código de 4 dígitos que enviamos para <span class="phone-number" aria-hidden="true">o seu e-mail</span><span class="sr-only">o endereço de e-mail cadastrado</span>',
  };
  ...
  function updateFeedbackDefault() {
    ...
    feedback.firstChild.textContent = "Não recebeu? ";

    if (resendSeconds > 0) {
      resendButton.disabled = true;
      resendButton.firstChild.textContent = "Reenviar em ";
      resendTime.hidden = false;
      resendTime.textContent = `0:${String(resendSeconds).padStart(2, "0")}`;
    } else {
      resendButton.disabled = false;
      resendButton.firstChild.textContent = "Reenviar código";
  ...
  function rejectWrongCode(cleanCode) {
    ...
    appState = "rejecting";
    setCode(cleanCode);
    showInputError("Código incorreto. Tente novamente.");
    focusInput();
  ...
  function revealSuccess({ instant = false } = {}) {
    ...
    title.textContent = "Verificado com sucesso";
    subtitle.textContent = "Seu e-mail foi verificado.";
    ...
    liveRegion.textContent = "Verificação concluída";
  ...
  async function runVerification(code = input.value) {
    ...
    if (cleanCode.length !== 4) {
      showInputError("Digite os quatro dígitos primeiro.");
      input.focus({ preventScroll: true });
      return;
    }
    ...
    liveRegion.textContent = "Verificando código";
    ...
    try {
      ...
      if (error?.name !== "AbortError") {
        ...
        showInputError("A verificação foi interrompida. Tente novamente.");
      }
    } finally {
  ...
  resendButton.addEventListener("click", () => {
    ...
    resendSeconds = 24;
    startResendTimer();
    liveRegion.textContent = "Um novo código foi enviado";
    animateTransient(
      demoNote,
      [
  ...
  continueButton.addEventListener("click", () => {
    ...
    arrow.textContent = "✓";
    liveRegion.textContent = "Pronto para continuar";
    window.dispatchEvent(new CustomEvent("vigia:otp-verified", { detail: { verified: true } }));
